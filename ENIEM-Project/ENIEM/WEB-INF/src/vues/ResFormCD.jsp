...
<body>
	...
	<div class="cadrSucc" style="margin-left:400px;margin-top:190px;width:480px;heigh350px;">
		...
		<div style="text-align:left; color:black; font-weight:bold; font-size:16px;font-family:Arial;margin-top:20px;fond-size:14px;">
			<span style="color:blue;">Intitulé de la formation :</span><%= request.getParameter("int_fcd").toString()%><br/><br/>
			<span style="color:blue;">Date de début de la formation :</span> <%= request.getParameter("jour_deb_fcd").toString()%>-<%= request.getParameter("mois_deb_fcd").toString()%>-<%= request.getParameter("annee_deb_fcd").toString()%><br/><br/>
			<span style="color:blue;">Date de fin de la formation :</span> <%= request.getParameter("jour_fin_fcd").toString()%>-<%= request.getParameter("mois_fin_fcd").toString()%>-<%= request.getParameter("annee_fin_fcd").toString()%><br/><br/>
			<span style="color:blue;">Thème de la formation :</span> <%= request.getParameter("theme_fcd").toString()%><br/><br/>
			<span style="color:blue;">Durée de la formation :</span><%= request.getParameter("dur_fcd").toString()%><br/><br/>
		</div>
		<div style="position:absolute; top:560px; left:1050px;">
			<a href="FORMCOURTE" title="Retour au formulaire d'ajout d'un contrat"><img src="Images/bouton-retour-accueil.png" alt="bouton-retour-accueil"></a>
		</div>
	</div>
